...
const apis: MockMethod[] = [
  ...
  // 用户+密码 登录
  {
    ...
    response: (options: Service.MockOption): Service.MockServiceResult<ApiAuth.Token | null> => {
      const { account = undefined, password = undefined } = options.body;
      ...
      const findItem = userModel.find(item => item.account === account && item.password === password);
  ...
  // 获取用户信息(请求头携带token, 根据token获取用户信息)
  {
    ...
    response: (options: Service.MockOption): Service.MockServiceResult<ApiAuth.UserInfo | null> => {
      ...
      const isInUser = userModel.some(item => {
        ...
        if (flag) {
          const { userId: itemUserId, account, userRole } = item;
          Object.assign(userInfo, { userId: itemUserId, account, userRole });
        }
        return flag;
      });
