...
export function createRequest(axiosConfig: AxiosRequestConfig, backendConfig?: Service.BackendResultConfig) {
   ...
   * - axiosConfig: axios配置
   */
  async function asyncRequest<T>(param: RequestParam): Promise<Service.RequestResult<T> | any> {
    const { url } = param;
    ...
    })) as Service.RequestResult<T> | any;
    return res;
  }
...
async function getRequestResponse(params: {
  ...
  if (res.error && res.error.code === 401) { useAuthStore().resetAuthStore() }
  return res;
}
